...
from BigmartsalesPrediction.app_entity.artifacts_entity import DataIngestionArtifact, DataValidationArtifact , DataTransformationArtifact
...
class DataTransformation:
    ...
    def data_transformation_load_data(self , file_path , selected_columns )->pd.DataFrame:
        try:
            ...
            load_df["Item_Fat_Content"] = load_df["Item_Fat_Content"].map({"Low Fat" : 'Low Fat' , "LF" : "Low Fat" , 'low fat' : "Low Fat" , "Regular" : "Regular"})
            load_df["Item_Identifier"] = load_df["Item_Identifier"].apply(lambda x:x[0:2]).map({"FD" : "Food" , "DR" : "Drink" , "NC" : "Non_consumable"})
            return load_df
        except Exception as e:
            raise App_Exception(e,sys) from e
